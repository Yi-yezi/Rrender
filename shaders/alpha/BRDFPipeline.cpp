...
void BRDFPipeline::Render(const std::shared_ptr<scene::Scene>& scene,
    ...
    m_BRDFShader->SetUniform("u_enableIBL", true);
    ...
    for (auto& light : scene->GetLights()) {
        switch (light->GetType()) {
            case graphics::LightType::Directional: {
                ...
                //m_BRDFShader->SetUniform(prefix + ".shadowLayer", dirLight->GetShadowLayer());
                // 方向光VP矩阵
            ...
            case graphics::LightType::Point: {
                ...
                //m_BRDFShader->SetUniform(prefix + ".shadowLayer", pointLight->GetShadowLayer());
                pointLightCount++;
                break;
            }
            case graphics::LightType::Spot: {
                ...
                //m_BRDFShader->SetUniform(prefix + ".shadowLayer", spotLight->GetShadowLayer());
                // 聚光灯VP矩阵
    ...
    //m_BRDFShader->SetUniform("u_dirShadowMap", 9);
    //m_BRDFShader->SetUniform("u_pointShadowMap", 10);
    //m_BRDFShader->SetUniform("u_spotShadowMap", 11);

    // 绑定纹理
    m_BRDFShader->SetUniform("u_albedoMap", graphics::TextureSlots::TEX_SLOT_ALBEDO);
    m_BRDFShader->SetUniform("u_normalMap", graphics::TextureSlots::TEX_SLOT_NORMAL);
    //m_BRDFShader->SetUniform("u_metallicRoughnessMap", graphics::TextureSlots::TEX_SLOT_ROUGHNESS);
    m_BRDFShader->SetUniform("u_specularGlossiness", graphics::TextureSlots::TEX_SLOT_SPECULAR);
    m_BRDFShader->SetUniform("u_aoMap", graphics::TextureSlots::TEX_SLOT_AO);
    m_BRDFShader->SetUniform("u_emissionMap", graphics::TextureSlots::TEX_SLOT_EMISSION);
    // 绘制实体
    for (auto& entity : scene->GetEntities()) {
        m_BRDFShader->SetUniform("u_model", entity->GetModelMatrix());
        entity->Draw();
    }
